...
100.times do |i|
  order = DeliveryOrder.create(
    ...
    address: FFaker::AddressBR.street_address,
    phone: '01210321114',
    created_at: Date.today - 1.months,
    created_at: Date.today - 1.months)
  sum = 0
  5.times do |i|
    product_order = ProductsDeliveryOrder.create(
...
100.times do |i|
  ...
  sum = 0
  5.times do |j|
    product_purchase = ProductsPurchaseOrder.create(
      product_id: Product.all.ids[rand(Product.count)],
      purchase_order_id: purchase.id,
      quantity: rand(5..50))
    sum += product_purchase.quantity * Product.find(product_purchase.product_id).price
  end
  purchase.update(total_price: sum)
